...
export const convertStateToAbbr = (input) => {
    ...
    if (strInput.length === 2) {
        ...
        return abbrToFullName[upStrInput] ? upStrInput : undefined;
    }
    var strStateToFind = strInput.toLowerCase().replace(/\ /g, '');
    var foundAbbr = fullNameToAbbr[strStateToFind];
    // console.log('foundAbbr', foundAbbr)
    return foundAbbr;
}

export const convertStateToFullName = (input) => {
    if (input === undefined) {
        return input;
    }
    var strInput = input.trim();
    if (strInput.length !== 2) {
        ...
        return abbrToFullName[convertStateToAbbr(strInput)];
    }
    var strStateToFind = strInput.toUpperCase().replace(/\ /g, '');
    var foundFullName = abbrToFullName[strStateToFind];
    // console.log('foundFullName', foundFullName)
